...
void kernel(const char* command) {
    ...
    for (vmiter it(kernel_pagetable); it.va() < MEMSIZE_PHYSICAL; it += PAGESIZE) {
        if (it.va() < PROC_START_ADDR && it.va() != CONSOLE_ADDR) {
            it.map(it.va(), PTE_P | PTE_W); // kernel memory
        } else if (it.va() != 0) {
            it.map(it.va(), PTE_P | PTE_W | PTE_U); // userspace memory
        } else {
...
void kfree(void* kptr) {
    ...
    if (pages[pa / PAGESIZE].refcount > 0) {
        pages[pa / PAGESIZE].refcount--;
    }
}

void pfree(x86_64_pagetable* pt){
...
void process_setup(pid_t pid, const char* program_name) {
    ...
    vmiter process_iterator(ptable[pid].pagetable);
    // Copy kernel mappings to new page table
    for (vmiter kernel_iterator(kernel_pagetable); 
        kernel_iterator.va() < PROC_START_ADDR; 
        kernel_iterator += PAGESIZE) {
        process_iterator.map(kernel_iterator.pa(), kernel_iterator.perm());
        process_iterator += PAGESIZE;
    }
    ...
    for (loader.reset(); loader.present(); ++loader) {
        for (uintptr_t a = round_down(loader.va(), PAGESIZE);
            ...
            int flags;
            process_iterator.find(a);
            if (loader.writable()) {
                flags = PTE_P | PTE_W | PTE_U;
            } else {
                flags =  PTE_P | PTE_U;
            }
            process_iterator.map(pa, flags);
        }
    }
    
    // We now copy instructions and data into memory that we just allocated.
    for (loader.reset(); loader.present(); ++loader) {
        process_iterator.find(loader.va());
        memset((void*) process_iterator.pa(), 0, loader.size());
        memcpy((void*) process_iterator.pa(), loader.data(), loader.data_size());
    }
    ...
    //pages[stack_pa / PAGESIZE].refcount = 1;

    process_iterator.find(stack_addr);
    process_iterator.map(stack_pa, PTE_P | PTE_W | PTE_U);
...
int syscall_page_alloc(uintptr_t addr) {
    ...
    int r = it.try_map(pa, PTE_P | PTE_W | PTE_U);
    if (r < 0) {
        kfree((void *) pa);
        return -1;
    }
...
pid_t syscall_fork() {
    int child_pid = 1;
    while (child_pid < NPROC && ptable[child_pid].state != P_FREE) {
        child_pid++;
    }
    if (child_pid == NPROC) {
        return -1; 
    ...
    for (vmiter it(current->pagetable); it.va() < MEMSIZE_VIRTUAL; it += PAGESIZE) {
        if (it.user() && (uintptr_t)it.va() != CONSOLE_ADDR && it.writable()) {
            ...
            int r = vmiter(child_pt).find(it.va()).try_map(pa, it.perm());
            if (r < 0) {
                pfree(child_pt);
                kfree(pa);
                return -1;
    ...
    ptable[child_pid].state = current->state;
    return child_pid;
}
